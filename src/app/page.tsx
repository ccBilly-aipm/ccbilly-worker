import { redirect } from "next/navigation";
import Link from "next/link";
import type { ReactNode } from "react";
import { ensureIndexReady } from "@/lib/index/bootstrap";
import { listByType } from "@/lib/index/queries";
import { dashboardStats } from "@/lib/dashboard/stats";
import { readSettings } from "@/lib/admin/settings";
import { readPreset } from "@/lib/preset/preset-service";
import { getLayout } from "@/lib/dashboard/layout-service";
import { WidgetGrid } from "@/features/dashboard/widget-grid";
import type { WidgetId } from "@/lib/dashboard/widgets";
import { TodayOrbit } from "@/features/dashboard/today-orbit";
import {
  LazyTrendArea,
  LazyDistributionPie,
} from "@/features/dashboard/lazy-charts";
import { ActivityHeatmap } from "@/features/dashboard/activity-heatmap";
import { QuickCaptureWidget } from "@/features/inbox/quick-capture-widget";
import { RequirementQueueWidget } from "@/features/pm/requirement-queue-widget";
import { CycleBurndownWidget } from "@/features/pm/cycle-burndown-widget";
import { ContentPipelineWidget } from "@/features/creator/content-pipeline-widget";
import { PublishCalendarWidget } from "@/features/creator/publish-calendar-widget";
import { localDateKey } from "@/lib/utils/date";

export const dynamic = "force-dynamic";

const WEEKDAYS = ["日", "一", "二", "三", "四", "五", "六"];

function greeting(h: number): string {
  if (h < 5) return "夜深了";
  if (h < 11) return "早上好";
  if (h < 14) return "中午好";
  if (h < 18) return "下午好";
  return "晚上好";
}

export default async function DashboardPage() {
  await ensureIndexReady();
  if (!readPreset().onboarded) redirect("/onboarding");

  const preset = readPreset().active;
  const layout = getLayout(preset);
  const stats = dashboardStats();
  const settings = readSettings();
  const tasks = listByType("task");
  const today = new Date();
  const dateLabel = `${localDateKey(today)} 星期${WEEKDAYS[today.getDay()]}`;
  const hello = greeting(today.getHours());

  const active = tasks.filter(
    (t) =>
      (t.data.kind ?? "task") === "task" &&
      t.data.status !== "done" &&
      t.data.status !== "archived",
  );

  // Server-render each widget's content into a map the client grid places.
  const content: Partial<Record<WidgetId, ReactNode>> = {
    "today-orbit": (
      <div className="flex items-center justify-center py-4">
        <TodayOrbit
          points={stats.orbit}
          completionRate={stats.completionRate}
          counts={stats.counts}
        />
      </div>
    ),
    "today-todo":
      active.length === 0 ? (
        <p className="py-6 text-center text-sm text-muted">
          没有待办任务。用右上角「新建」创建第一个吧。
        </p>
      ) : (
        <ul className="space-y-2">
          {active.slice(0, 7).map((t) => (
            <Link
              key={t.filePath}
              href={`/tasks?open=${encodeURIComponent(t.slug)}`}
              className="flex items-center justify-between rounded-lg px-3 py-2 text-sm hover:bg-[rgb(var(--glass-bg)/0.06)]"
            >
              <span className="truncate text-fg">{String(t.data.title)}</span>
              <span className="rounded bg-[rgb(var(--glass-bg)/0.1)] px-1.5 py-0.5 text-xs text-muted">
                {String(t.data.priority ?? "P2")}
              </span>
            </Link>
          ))}
        </ul>
      ),
    "trend-14": <LazyTrendArea data={stats.trend14} />,
    distribution: <LazyDistributionPie data={stats.distribution} />,
    heatmap: <ActivityHeatmap data={stats.heatmap} />,
    "quick-capture": <QuickCaptureWidget />,
    "requirement-queue": <RequirementQueueWidget />,
    "cycle-burndown": <CycleBurndownWidget />,
    "content-pipeline": <ContentPipelineWidget />,
    "publish-calendar": <PublishCalendarWidget />,
  };

  return (
    <div className="mx-auto max-w-6xl space-y-4">
      <header className="space-y-1">
        <h1 className="font-display text-2xl font-semibold text-fg md:text-3xl">
          {hello}，<span className="text-brand-gradient">{settings.displayName}</span>
        </h1>
        <p className="text-sm text-muted tabular">{dateLabel}</p>
      </header>

      <WidgetGrid initialLayout={layout} content={content} />
    </div>
  );
}
